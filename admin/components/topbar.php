...
if (!function_exists('renderAdminTopbar')) {
	function renderAdminTopbar(string $username = 'Admin', string $title = ''): void {
		...
		document.addEventListener('DOMContentLoaded', function(){
			...
			// Global ripple for modern action buttons
			...
			// Attach ripple and click micro animation
			document.querySelectorAll('.modern-btn').forEach(function(el){
				el.addEventListener('click', createRipple);
			});
			document.addEventListener('click', function(e){
				const mb = e.target.closest('.modern-btn');
				if (!mb) return;
