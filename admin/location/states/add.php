...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	try {
		...
		$stmt && $stmt->bind_param('ss', $name, $image_url);

		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to add state'); }
		$state_id = $mysqli->insert_id;
		$stmt->close();

		logActivity($mysqli, 'Added state', 'Name: ' . $name);
...
<head>
	...
	<title>Add State - Big Deal</title>
...
<body>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Add New State</h2>
					<p class="text-muted mb-0">Create a new state for location hierarchy</p>
				</div>
					...
					<div class="col-lg-12">
						<div class="card mb-4">
							<div class="card-body">
								...
								<div class="mb-3">
									<label class="form-label">State Image (optional)</label>
	...
	<script>
		...
		document.getElementById('stateForm').addEventListener('submit', function(e){
			...
			if (name.length < 2) { e.preventDefault(); alert('Please enter a valid state name'); }
		});
	</script>
</body>
</html>
