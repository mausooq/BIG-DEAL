...
$stmt = $mysqli->prepare('SELECT id, name, image_url FROM states WHERE id = ?');
$stmt && $stmt->bind_param('i', $id) && $stmt->execute();
$state = $stmt ? $stmt->get_result()->fetch_assoc() : null;
$stmt && $stmt->close();
if (!$state) { header('Location: index.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		...
		if ($name === '') { throw new Exception('State name is required'); }
		...
		if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
			...
			$filename = 'state_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
		...
		$stmt && $stmt->bind_param('ssi', $name, $image_url, $id);
		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to update state'); }
		$stmt && $stmt->close();

		logActivity($mysqli, 'Updated state', 'ID: ' . $id . ', Name: ' . $name);
...
<head>
	...
	<title>Edit State - Big Deal</title>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Edit State</h2>
					<p class="text-muted mb-0">Update the state information</p>
				</div>
				<a href="index.php" class="btn btn-outline-secondary"><i class="fa-solid fa-arrow-left me-2"></i>Back to States</a>
			</div>
			...
			<form method="post" enctype="multipart/form-data" id="stateForm">
				<div class="row">
					<div class="col-lg-12">
						<div class="card mb-4">
							<div class="card-body">
								<div class="mb-3">
									...
									<input type="text" class="form-control" name="name" value="<?php echo htmlspecialchars($state['name']); ?>" required>
								</div>
								<div class="mb-3">
									<label class="form-label">Replace State Image (optional)</label>
									...
									<div class="preview mt-2" id="preview" style="display:block;">
										<?php if (!empty($state['image_url'])): ?>
											...
											?>
											<img src="<?php echo htmlspecialchars($src); ?>" alt="Current State Image">
										<?php else: ?>
