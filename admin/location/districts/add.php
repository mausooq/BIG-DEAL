...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		...
		$stmt && $stmt->bind_param('iss', $state_id, $name, $image_url);

		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to add district'); }
		$district_id = $mysqli->insert_id;
		$stmt->close();

		logActivity($mysqli, 'Added district', 'Name: ' . $name . ', State ID: ' . $state_id);
...
<head>
	...
	<title>Add District - Big Deal</title>
...
<body>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Add New District</h2>
					<p class="text-muted mb-0">Create a new district for location hierarchy</p>
				</div>
					...
					<div class="col-lg-12">
						<div class="card mb-4">
							<div class="card-body">
								...
								<div class="mb-3 mt-3">
									<label class="form-label">District Image (optional)</label>
	...
	<script>
		...
		document.getElementById('districtForm').addEventListener('submit', function(e){
			...
			if (name.length < 2 || !stateId) { e.preventDefault(); alert('Please enter a valid district name and select a state'); }
		});
	</script>
</body>
</html>
