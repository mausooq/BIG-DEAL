...
$stmt = $mysqli->prepare('SELECT id, state_id, name, image_url FROM districts WHERE id = ?');
$stmt && $stmt->bind_param('i', $id) && $stmt->execute();
$district = $stmt ? $stmt->get_result()->fetch_assoc() : null;
$stmt && $stmt->close();
if (!$district) { header('Location: index.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		...
		if ($name === '' || $state_id <= 0) { throw new Exception('District name and state are required'); }
		...
		if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
			...
			$filename = 'district_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
		...
		$stmt && $stmt->bind_param('issi', $state_id, $name, $image_url, $id);
		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to update district'); }
		$stmt && $stmt->close();

		logActivity($mysqli, 'Updated district', 'ID: ' . $id . ', Name: ' . $name);
...
<head>
	...
	<title>Edit District - Big Deal</title>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Edit District</h2>
					<p class="text-muted mb-0">Update the district information</p>
				</div>
				<a href="index.php" class="btn btn-outline-secondary"><i class="fa-solid fa-arrow-left me-2"></i>Back to Districts</a>
			</div>
			...
			<form method="post" enctype="multipart/form-data" id="districtForm">
				<div class="row">
					<div class="col-lg-12">
						<div class="card mb-4">
							<div class="card-body">
									...
									<div class="col-md-6">
										...
										<select class="form-control" name="state_id" required>
											...
											<?php while ($state = $states->fetch_assoc()): ?>
												<option value="<?php echo $state['id']; ?>" <?php echo $state['id'] == $district['state_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($state['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-md-6">
										<label class="form-label">District Name <span class="text-danger">*</span></label>
										<input type="text" class="form-control" name="name" value="<?php echo htmlspecialchars($district['name']); ?>" required>
									</div>
								</div>
								<div class="mb-3 mt-3">
									<label class="form-label">Replace District Image (optional)</label>
									...
									<div class="preview mt-2" id="preview" style="display:block;">
										<?php if (!empty($district['image_url'])): ?>
											...
											?>
											<img src="<?php echo htmlspecialchars($src); ?>" alt="Current District Image">
										<?php else: ?>
