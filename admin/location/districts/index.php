    ...
    if ($action === 'add_district') {
        ...
        if (!$name || !$state_id) {
            ...
            if (!isset($_SESSION['error_message'])) {
                $stmt = $mysqli->prepare('INSERT INTO districts (state_id, name, image_url) VALUES (?, ?, ?)');
                if ($stmt) {
                    $stmt->bind_param('iss', $state_id, $name, $image_filename);
                    if ($stmt->execute()) {
                        $district_id = $mysqli->insert_id;
                        logActivity($mysqli, 'Added district', 'Name: ' . $name . ', State ID: ' . $state_id . ', ID: ' . $district_id);
        ...
        if (!$id || !$name || !$state_id) {
            ...
            if (!isset($_SESSION['error_message'])) {
                $stmt = $mysqli->prepare('UPDATE districts SET state_id = ?, name = ?, image_url = ? WHERE id = ?');
                $stmt->bind_param('issi', $state_id, $name, $image_filename, $id);
                
                if ($stmt) {
                    if ($stmt->execute()) {
                        if ($stmt->affected_rows > 0) {
                            logActivity($mysqli, 'Updated district', 'Name: ' . $name . ', State ID: ' . $state_id . ', ID: ' . $id);
        ...
        $id = (int)($_POST['id'] ?? 0);
        if ($id) {
            // Get district details for logging
...
// Get districts with search and pagination
$mysqli = db();
$search = $_GET['search'] ?? '';
...
$sql = 'SELECT d.id, d.name, d.image_url, s.name as state_name FROM districts d 
        LEFT JOIN states s ON d.state_id = s.id' . $whereClause . ' ORDER BY s.name ASC, d.name ASC LIMIT ? OFFSET ?';
$types .= 'ii';
$params[] = $limit;
$params[] = $offset;
...
$stmt && $stmt->execute();
$districts = $stmt ? $stmt->get_result() : $mysqli->query('SELECT d.id, d.name, d.image_url, s.name as state_name FROM districts d LEFT JOIN states s ON d.state_id = s.id ORDER BY s.name ASC, d.name ASC LIMIT 10');

// Count
$countSql = 'SELECT COUNT(*) FROM districts d' . $whereClause;
$countStmt = $mysqli->prepare($countSql);
if ($countStmt && ($search || $state_filter)) {
    $countParams = [];
    $countTypes = '';
    if ($search) {
        $countTypes .= 's';
...
<head>
    ...
    <title>Districts - Big Deal</title>
...
<body>
    ...
    <div class="content">
        <?php require_once __DIR__ . '/../../components/topbar.php'; renderAdminTopbar($_SESSION['admin_username'] ?? 'Admin', 'Districts'); ?>
                ...
                <div class="row-top">
                    <form class="d-flex flex-grow-1" method="get">
                        <div class="input-group me-2">
                            ...
                            <input type="text" class="form-control" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search districts by name">
                        </div>
                        <select class="form-control me-2" name="state_id" style="max-width: 200px;">
                    ...
                    <div class="card quick-card mb-4">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <div class="h6 mb-0">Districts (<?php echo $totalCount; ?>)</div>
                                    ...
                                    <tbody>
                                        <?php if ($districts && $districts->num_rows > 0): ?>
                                            <?php while ($district = $districts->fetch_assoc()): ?>
                                                <?php
                                                // Get city count for this district
                                                ...
                                                <tr data-district='<?php echo json_encode($district, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT); ?>'>
                                                    ...
                                                    <td class="fw-semibold">
                                                        <a href="../cities/index.php?district_id=<?php echo $district['id']; ?>" class="badge bg-light text-dark border text-decoration-none" title="View cities in this district">
                                                            <?php echo htmlspecialchars($district['name']); ?>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <span class="badge badge-soft"><?php echo htmlspecialchars($district['state_name']); ?></span>
                                                    </td>
                                                    <td>
                                                        <span class="badge badge-soft"><?php echo $cityCount; ?> cities</span>
                                                    </td>
                                                    <td class="actions-cell">
                                                        <button class="btn btn-sm btn-outline-secondary btn-edit me-1" data-bs-toggle="modal" data-bs-target="#editDistrictModal" title="Edit District"><i class="fa-solid fa-pen"></i></button>
                                                        <button class="btn btn-sm btn-outline-danger btn-delete" data-bs-toggle="modal" data-bs-target="#deleteDistrictModal" title="Delete District"><i class="fa-solid fa-trash"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endwhile; ?>
                                        <?php else: ?>
                            ...
                            <nav aria-label="Districts pagination">
                                <ul class="pagination justify-content-center">
                                    ...
                                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                        <a class="page-link" href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&state_id=<?php echo urlencode($state_filter); ?>"><?php echo $i; ?></a>
                                    </li>
                                    <?php endfor; ?>
                                </ul>
                            </nav>
    ...
    <div class="modal fade" id="deleteDistrictModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete the district "<span id="delete_district_name"></span>"?</p>
                    <p class="text-muted small">This action will also delete all associated cities and towns. This action cannot be undone.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        document.addEventListener('DOMContentLoaded', function(){
            document.querySelectorAll('.btn-edit').forEach(btn => {
                btn.addEventListener('click', function(){
                    ...
                    const data = JSON.parse(tr.getAttribute('data-district'));
            ...
            document.querySelectorAll('.btn-delete').forEach(btn => {
                btn.addEventListener('click', function(){
                    ...
                    document.getElementById('delete_district_name').textContent = data.name;
                });
            });
