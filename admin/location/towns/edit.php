...
// Fetch existing town
$stmt = $mysqli->prepare('SELECT t.id, t.city_id, t.name, t.image_url, c.district_id, d.state_id 
                         FROM towns t 
                         LEFT JOIN cities c ON t.city_id = c.id 
                         LEFT JOIN districts d ON c.district_id = d.id 
                         WHERE t.id = ?');
$stmt && $stmt->bind_param('i', $id) && $stmt->execute();
$town = $stmt ? $stmt->get_result()->fetch_assoc() : null;
$stmt && $stmt->close();
if (!$town) { header('Location: index.php'); exit(); }

// Get districts and cities for the town's state/district
$districts = $mysqli->query("SELECT id, name FROM districts WHERE state_id = " . $town['state_id'] . " ORDER BY name ASC");
$cities = $mysqli->query("SELECT id, name FROM cities WHERE district_id = " . $town['district_id'] . " ORDER BY name ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		...
		$stmt && $stmt->bind_param('issi', $city_id, $name, $image_url, $id);
		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to update town'); }
		$stmt && $stmt->close();

		logActivity($mysqli, 'Updated town', 'ID: ' . $id . ', Name: ' . $name);
...
<head>
	...
	<title>Edit Town - Big Deal</title>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Edit Town</h2>
					<p class="text-muted mb-0">Update the town information</p>
				</div>
									...
									<div class="col-md-4">
										...
										<select class="form-control" name="state_id" id="state_id" required>
											...
											<?php while ($state = $states->fetch_assoc()): ?>
												<option value="<?php echo $state['id']; ?>" <?php echo $state['id'] == $town['state_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($state['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-md-4">
										...
										<select class="form-control" name="district_id" id="district_id" required>
											...
											<?php while ($district = $districts->fetch_assoc()): ?>
												<option value="<?php echo $district['id']; ?>" <?php echo $district['id'] == $town['district_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($district['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-md-4">
										...
										<select class="form-control" name="city_id" id="city_id" required>
											...
											<?php while ($city = $cities->fetch_assoc()): ?>
												<option value="<?php echo $city['id']; ?>" <?php echo $city['id'] == $town['city_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($city['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-12">
										<label class="form-label">Town Name <span class="text-danger">*</span></label>
										<input type="text" class="form-control" name="name" value="<?php echo htmlspecialchars($town['name']); ?>" required>
									</div>
								</div>
								<div class="mb-3 mt-3">
									<label class="form-label">Replace Town Image (optional)</label>
										...
										<?php if (!empty($town['image_url'])): ?>
											<?php 
												...
												if (strpos($src,'http://')!==0 && strpos($src,'https://')!==0) { 
													$src = '../../../uploads/locations/' . $src; 
												} 
											?>
											<img src="<?php echo htmlspecialchars($src); ?>" alt="Current Town Image">
										<?php else: ?>
