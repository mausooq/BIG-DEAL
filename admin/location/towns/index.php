    ...
    if ($action === 'add_town') {
        ...
        if (!$name || !$city_id) {
            ...
            if (!isset($_SESSION['error_message'])) {
                ...
                if ($stmt) {
                    $stmt->bind_param('iss', $city_id, $name, $image_filename);
                    if ($stmt->execute()) {
                        $town_id = $mysqli->insert_id;
                        logActivity($mysqli, 'Added town', 'Name: ' . $name . ', City ID: ' . $city_id . ', ID: ' . $town_id);
        ...
        if (!$id || !$name || !$city_id) {
            ...
            if (!isset($_SESSION['error_message'])) {
                $stmt = $mysqli->prepare('UPDATE towns SET city_id = ?, name = ?, image_url = ? WHERE id = ?');
                $stmt->bind_param('issi', $city_id, $name, $image_filename, $id);
                
                if ($stmt) {
                    if ($stmt->execute()) {
                        if ($stmt->affected_rows > 0) {
                            logActivity($mysqli, 'Updated town', 'Name: ' . $name . ', City ID: ' . $city_id . ', ID: ' . $id);
...
// Get towns with search and pagination
$mysqli = db();
$search = $_GET['search'] ?? '';
$state_filter = $_GET['state_id'] ?? '';
$district_filter = $_GET['district_id'] ?? '';
$city_filter = $_GET['city_id'] ?? '';
...
if ($search || $state_filter || $district_filter || $city_filter) {
    $conditions = [];
    if ($search) {
...
$sql = 'SELECT t.id, t.name, t.image_url, c.name as city_name, d.name as district_name, s.name as state_name 
        FROM towns t 
        LEFT JOIN cities c ON t.city_id = c.id 
        LEFT JOIN districts d ON c.district_id = d.id 
        LEFT JOIN states s ON d.state_id = s.id' . $whereClause . ' ORDER BY s.name ASC, d.name ASC, c.name ASC, t.name ASC LIMIT ? OFFSET ?';
$types .= 'ii';
$params[] = $limit;
$params[] = $offset;
...
$stmt && $stmt->execute();
$towns = $stmt ? $stmt->get_result() : $mysqli->query('SELECT t.id, t.name, t.image_url, c.name as city_name, d.name as district_name, s.name as state_name FROM towns t LEFT JOIN cities c ON t.city_id = c.id LEFT JOIN districts d ON c.district_id = d.id LEFT JOIN states s ON d.state_id = s.id ORDER BY s.name ASC, d.name ASC, c.name ASC, t.name ASC LIMIT 10');

// Count
$countSql = 'SELECT COUNT(*) FROM towns t LEFT JOIN cities c ON t.city_id = c.id LEFT JOIN districts d ON c.district_id = d.id LEFT JOIN states s ON d.state_id = s.id' . $whereClause;
$countStmt = $mysqli->prepare($countSql);
if ($countStmt && ($search || $state_filter || $district_filter || $city_filter)) {
    $countParams = [];
    $countTypes = '';
    if ($search) {
        $countTypes .= 's';
...
// Get states, districts, and cities for filter dropdowns
...
if ($district_filter) {
    $cities = $mysqli->query("SELECT id, name FROM cities WHERE district_id = " . (int)$district_filter . " ORDER BY name ASC");
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="content">
        <?php require_once __DIR__ . '/../../components/topbar.php'; renderAdminTopbar($_SESSION['admin_username'] ?? 'Admin', 'Towns'); ?>
                ...
                <div class="row-top">
                    <form class="d-flex flex-grow-1" method="get">
                        <div class="input-group me-2">
                            ...
                            <input type="text" class="form-control" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search towns by name">
                        </div>
                        <select class="form-control me-2" name="state_id" id="state_filter" style="max-width: 120px;">
                        ...
                        <select class="form-control me-2" name="district_id" id="district_filter" style="max-width: 120px;">
                        ...
                        <select class="form-control me-2" name="city_id" id="city_filter" style="max-width: 120px;">
                            <option value="">All Cities</option>
                            <?php while ($city = $cities->fetch_assoc()): ?>
                                <option value="<?php echo $city['id']; ?>" <?php echo $city['id'] == $city_filter ? 'selected' : ''; ?>><?php echo htmlspecialchars($city['name']); ?></option>
                            <?php endwhile; ?>
                        </select>
                    ...
                    <div class="card quick-card mb-4">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <div class="h6 mb-0">Towns (<?php echo $totalCount; ?>)</div>
                                        ...
                                        <?php if ($towns && $towns->num_rows > 0): ?>
                                            <?php while ($town = $towns->fetch_assoc()): ?>
                                                <tr data-town='<?php echo json_encode($town, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT); ?>'>
                                                    ...
                                                    <td>
                                                        <span class="badge badge-soft"><?php echo htmlspecialchars($town['city_name']); ?></span>
                                                    </td>
                                                    <td>
                                                        <span class="badge badge-soft"><?php echo htmlspecialchars($town['district_name']); ?></span>
                                                    </td>
                                                    <td>
                                                        <span class="badge badge-soft"><?php echo htmlspecialchars($town['state_name']); ?></span>
                                                    </td>
                                                    <td class="actions-cell">
                                                        <button class="btn btn-sm btn-outline-secondary btn-edit me-1" data-bs-toggle="modal" data-bs-target="#editTownModal" title="Edit Town"><i class="fa-solid fa-pen"></i></button>
                                                        <button class="btn btn-sm btn-outline-danger btn-delete" data-bs-toggle="modal" data-bs-target="#deleteTownModal" title="Delete Town"><i class="fa-solid fa-trash"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endwhile; ?>
                                        <?php else: ?>
                            ...
                            <nav aria-label="Towns pagination">
                                <ul class="pagination justify-content-center">
                                    ...
                                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                        <a class="page-link" href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&state_id=<?php echo urlencode($state_filter); ?>&district_id=<?php echo urlencode($district_filter); ?>&city_id=<?php echo urlencode($city_filter); ?>"><?php echo $i; ?></a>
                                    </li>
                                    <?php endfor; ?>
                                </ul>
                            </nav>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="city_id" id="modal_city_id" required>
    ...
    <div class="modal fade" id="deleteTownModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete the town "<span id="delete_town_name"></span>"?</p>
                ...
                <div class="modal-footer">
                    ...
                    <form method="POST" style="display: inline;">
                        ...
                        <button type="submit" class="btn btn-danger">Delete Town</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            ...
            function setupStateChangeHandler(stateSelect, districtSelect, citySelect) {
            ...
            function setupDistrictChangeHandler(districtSelect, citySelect) {
            ...
            setupStateChangeHandler(document.getElementById('state_filter'), document.getElementById('district_filter'), document.getElementById('city_filter'));
            setupStateChangeHandler(document.getElementById('modal_state_id'), document.getElementById('modal_district_id'), document.getElementById('modal_city_id'));
            setupStateChangeHandler(document.getElementById('edit_state_id'), document.getElementById('edit_district_id'), document.getElementById('edit_city_id'));
            
            setupDistrictChangeHandler(document.getElementById('district_filter'), document.getElementById('city_filter'));
            setupDistrictChangeHandler(document.getElementById('modal_district_id'), document.getElementById('modal_city_id'));
            setupDistrictChangeHandler(document.getElementById('edit_district_id'), document.getElementById('edit_city_id'));

            document.querySelectorAll('.btn-edit').forEach(btn => {
                btn.addEventListener('click', function(){
                    const tr = this.closest('tr');
                    const data = JSON.parse(tr.getAttribute('data-town'));
                    ...
                    // Trigger district and city load for edit modal
            ...
            document.querySelectorAll('.btn-delete').forEach(btn => {
                btn.addEventListener('click', function(){
                    ...
                    document.getElementById('delete_town_name').textContent = data.name;
                });
            });
