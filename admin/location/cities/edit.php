...
// Fetch existing city
$stmt = $mysqli->prepare('SELECT c.id, c.district_id, c.name, c.image_url, d.state_id FROM cities c LEFT JOIN districts d ON c.district_id = d.id WHERE c.id = ?');
$stmt && $stmt->bind_param('i', $id) && $stmt->execute();
$city = $stmt ? $stmt->get_result()->fetch_assoc() : null;
$stmt && $stmt->close();
if (!$city) { header('Location: index.php'); exit(); }

// Get districts for the city's state
$districts = $mysqli->query("SELECT id, name FROM districts WHERE state_id = " . $city['state_id'] . " ORDER BY name ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		...
		if ($name === '' || $district_id <= 0) { throw new Exception('City name and district are required'); }
		...
		if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
			...
			$filename = 'city_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
		...
		$stmt && $stmt->bind_param('issi', $district_id, $name, $image_url, $id);
		if (!$stmt || !$stmt->execute()) { throw new Exception('Failed to update city'); }
		$stmt && $stmt->close();

		logActivity($mysqli, 'Updated city', 'ID: ' . $id . ', Name: ' . $name);
...
<head>
	...
	<title>Edit City - Big Deal</title>
	...
	<div class="content">
		...
		<div class="container-fluid p-4">
			<div class="d-flex align-items-center justify-content-between mb-4">
				<div>
					<h2 class="h4 mb-1 fw-semibold">Edit City</h2>
					<p class="text-muted mb-0">Update the city information</p>
				</div>
				<a href="index.php" class="btn btn-outline-secondary"><i class="fa-solid fa-arrow-left me-2"></i>Back to Cities</a>
			</div>
					...
					<div class="col-lg-12">
						<div class="card mb-4">
							<div class="card-body">
									...
									<div class="col-md-6">
										...
										<select class="form-control" name="state_id" id="state_id" required>
											...
											<?php while ($state = $states->fetch_assoc()): ?>
												<option value="<?php echo $state['id']; ?>" <?php echo $state['id'] == $city['state_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($state['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-md-6">
										...
										<select class="form-control" name="district_id" id="district_id" required>
											...
											<?php while ($district = $districts->fetch_assoc()): ?>
												<option value="<?php echo $district['id']; ?>" <?php echo $district['id'] == $city['district_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($district['name']); ?></option>
											<?php endwhile; ?>
										</select>
									</div>
									<div class="col-12">
								...
								<div class="mb-3 mt-3">
									<label class="form-label">Replace City Image (optional)</label>
									...
									<div class="preview mt-2" id="preview" style="display:block;">
										<?php if (!empty($city['image_url'])): ?>
											...
											?>
											<img src="<?php echo htmlspecialchars($src); ?>" alt="Current City Image">
										<?php else: ?>
